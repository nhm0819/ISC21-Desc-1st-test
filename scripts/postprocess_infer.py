...
def load_descriptor_h5(descs_submission_path):
    ...
    with h5py.File(descs_submission_path, "r") as f:
        ...
        query_paths = f["query_paths"][:]
        reference_paths = f["reference_paths"][:]
    ...
    return query, reference, train, query_ids, reference_ids, query_paths, reference_paths
...
def main():
    ...
    for ver in versions:
        ...
        _query, _reference, _, query_ids, reference_ids, query_paths, reference_paths = load_descriptor_h5(f'../exp/{ver}/extract/fb-isc-submission-phase2.h5')
    ...
    with h5py.File(out, 'w') as f:
        ...
        f.create_dataset('query_paths', data=query_paths,
                         dtype=sdtype)
        f.create_dataset('reference_paths', data=reference_paths,
                         dtype=sdtype)

    submission = pd.DataFrame(columns=['query_id', 'reference_id', 'query_path', 'reference_path', 'score'])
    submission['query_id'] = np.repeat(query_ids, 10)
    submission['reference_id'] = np.array(reference_ids)[reference_ind.ravel()]
    submission['query_path'] = np.repeat(query_paths, 10).astype('str')
    submission['reference_path'] = np.array(reference_paths)[reference_ind.ravel()].astype('str')
